...
static int heap_extend(size_t size) {
    ...
    // Allocate physical pages for the heap from KERNEL type (LOW zone)
    for (size_t i = 0; i < pages_needed; i++) {
        uint64_t phys = pmm_alloc_page_for_kernel();
        if (!phys) {
...
void heap_init(void) {
    ...
    if (!heap_extend(HEAP_INITIAL_SIZE)) {
    ...
    header->size  = HEAP_INITIAL_SIZE - sizeof(heap_header_t);
    header->used  = 0;
    header->next  = NULL;
    header->prev  = NULL;
    free_list     = header;
    ...
    serial_print(" size=");
    serial_print_dec(HEAP_INITIAL_SIZE / 1024);
    serial_print(" KB\n");
}
